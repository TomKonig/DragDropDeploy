...
async function startEphemeralPostgres(): Promise<string | undefined> {
  try {
    ...
    console.warn('[jest.global-setup] Failed to start testcontainers Postgres, falling back to external DATABASE_URL. Reason:', (err as any)?.message);
    return undefined;
  }
}

async function waitForPostgres(databaseUrl: string, attempts = 30, delayMs = 500): Promise<void> {
...
export default async () => {
  ...
  if (!process.env.DATABASE_URL) {
    ...
    if (url) {
      ...
      throw new Error('No DATABASE_URL provided and unable to start ephemeral Postgres. Set DATABASE_URL for tests.');
    }
  }
