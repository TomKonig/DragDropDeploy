...
export class UpdateProjectDto {
	@ApiPropertyOptional({ minLength: 1, maxLength: 100, example: 'renamed-project' })
	@IsOptional()
	@IsString()
	@Length(1, 100)
	name?: string;
	...
	@ApiPropertyOptional({ description: 'Disable HTML/CSS/JS minification for this project' })
	@IsOptional()
	@IsBoolean()
	optOutMinify?: boolean;

	@ApiPropertyOptional({ description: 'Additional build CLI flags (allow-listed)', maxItems: 20, example: ['--force', '--verbose'] })
	@IsOptional()
	@IsArray()
	@ArrayMaxSize(20)
	@ArrayUnique()
