...
describe("Build history & concurrency (e2e)", () => {
  ...
  it("creates sequential versions and prevents parallel active builds", async () => {
    const token = await registerAndAuth(
      `user_build_hist_${Date.now()}@example.com`,
    );
    ...
    await new Promise((r) => {
      const t = setTimeout(r, FIRST_DELAY_MS);
    ...
    for (;;) {
      const res = await request(app.getHttpServer())
        ...
        .set("Authorization", `Bearer ${token}`);
      if (res.status === 200 && ["SUCCESS", "FAILED"].includes(res.body.status))
        break;
      if (Date.now() - pollStart > MAX_WAIT_MS) {
        throw new Error(
          `Timed out waiting for initial build to finish (last status=${res.body?.status})`,
        );
      }
      await new Promise((r) => {
