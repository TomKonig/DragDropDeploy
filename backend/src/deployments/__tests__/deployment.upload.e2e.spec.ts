...
describe("Deployment Upload E2E", () => {
  ...
  beforeAll(async () => {
    ...
    const project = await prisma.project.create({
      data: {
        name: "UploadProj",
        ownerId: (await prisma.user.findUnique({ where: { email } }))!.id,
      },
    });
  ...
  it("happy path: uploads small zip", async () => {
    ...
    // dynamic import fs for type safety
    const { existsSync } = await import("fs");
    expect(existsSync(res.body.artifactPath)).toBe(true);
  });
  ...
  it("creates a build job when uploading", async () => {
    ...
    expect(res.status).toBe(201);
    // deployment id implicitly validated by presence of field
    expect(res.body.status).toBe("BUILDING");
    ...
    const recent = buildJobs.some(
      (j) => Date.now() - j.createdAt.getTime() < 60000,
    );
  ...
  it("rejects oversize zip", async () => {
    ...
    const old = process.env.MAX_UPLOAD_MB;
    process.env.MAX_UPLOAD_MB = "0.05"; // ~51KB
    const res = await request(app.getHttpServer())
      .post("/deployments/upload")
      .set("Authorization", `Bearer ${authToken}`)
      .field("projectId", projectId)
