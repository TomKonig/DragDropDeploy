...
describe('Deployments end-to-end', () => {
  ...
  it('deploy v1 then v2 then rollback to v1 (auth)', async () => {
    ...
    const project = await prisma.project.create({ data: { name: 'Demo', ownerId: user!.id } });
  ...
  await request(app.getHttpServer()).post(`/deployments/${res1.body.deploymentId}/activate`).set('Authorization', `Bearer ${token}`).expect(201);

    // Serve file
  const serve1 = await request(app.getHttpServer()).get(`/deployments/project/${project.id}/site/`).set('Authorization', `Bearer ${token}`).expect(200);
  ...
  await request(app.getHttpServer()).post(`/deployments/${res2.body.deploymentId}/activate`).set('Authorization', `Bearer ${token}`).expect(201);

  const serve2 = await request(app.getHttpServer()).get(`/deployments/project/${project.id}/site/`).set('Authorization', `Bearer ${token}`).expect(200);
    expect(serve2.text).toContain('Version2');

    // Rollback (should pick previous inactive)
  await request(app.getHttpServer()).post(`/deployments/project/${project.id}/rollback`).set('Authorization', `Bearer ${token}`).expect(201);
  const serveRollback = await request(app.getHttpServer()).get(`/deployments/project/${project.id}/site/`).set('Authorization', `Bearer ${token}`).expect(200);
