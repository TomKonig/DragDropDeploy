...
export class UploadExtractionService {
  ...
  async validateAndStage(projectId: string, archiveBuffer: Buffer, originalName: string): Promise<ExtractResult> {
    if (!projectId) throw new BadRequestException('projectId is required');
    if (!/\.zip$/i.test(originalName)) throw new BadRequestException('Only .zip uploads supported initially');
    ...
    if (archiveBuffer.length < 4 || archiveBuffer[0] !== 0x50 || archiveBuffer[1] !== 0x4b) {
      throw new BadRequestException('Invalid zip signature (missing PK header)');
    }
  ...
  const entries = Object.values(zip.files) as any[];
    ...
    if (entries.length > this.maxEntries) throw new BadRequestException('Too many files');
    const tempDir = path.join(os.tmpdir(), 'ddd-upload-' + randomUUID());
    await fs.promises.mkdir(tempDir, { recursive: true });
    let totalUncompressedBytes = 0; let fileCount = 0;
  for (const entry of entries) {
      ...
      if (unixName.startsWith('/')) throw new BadRequestException('Absolute path not allowed');
      ...
      if (/\.\./.test(normalized)) throw new BadRequestException('Path traversal detected');
      const destPath = path.join(tempDir, normalized);
      const rel = path.relative(tempDir, destPath);
      if (rel.startsWith('..')) throw new BadRequestException('Path traversal detected');
      const parent = path.dirname(destPath);
      await fs.promises.mkdir(parent, { recursive: true });
      const content = await entry.async('nodebuffer');
      totalUncompressedBytes += content.length;
      if (totalUncompressedBytes / zipSize > this.maxCompressionRatio) {
        throw new BadRequestException('Compression ratio too high (possible zip bomb)');
      }
