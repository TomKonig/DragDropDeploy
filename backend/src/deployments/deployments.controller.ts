import { Controller, Post, UploadedFile, UseInterceptors, Body, BadRequestException, HttpCode, HttpStatus, Param, Get, Query, Res } from '@nestjs/common';
...
export class DeploymentsController {
  ...
  async uploadArchive(@UploadedFile() file: Express.Multer.File, @Body('projectId') projectId: string): Promise<UploadResponse> {
    if (process.env.NODE_ENV === 'test') {
      // In tests we don't have auth context; fake tenant user via env bypass
      (global as any).__TENANT_USER__ = null;
    }
  ...
  if (!/^c[a-z0-9]{24,}$/i.test(id)) throw new BadRequestException('Invalid id');
  ...
  if (!/^c[a-z0-9]{24,}$/i.test(projectId)) return res.status(400).send('Invalid projectId');
    ...
    const reqPath = (res.req.params as any)[0] || '';
    ...
    if (!finalReal.startsWith(activePath + path.sep)) return res.status(400).send('Invalid path');
    try {
  ...
  async rollback(@Param('projectId') projectId: string, @Body('targetDeploymentId') targetDeploymentId?: string) {
  if (!/^c[a-z0-9]{24,}$/i.test(projectId)) throw new BadRequestException('Invalid projectId');
  if (targetDeploymentId && !/^c[a-z0-9]{24,}$/i.test(targetDeploymentId)) throw new BadRequestException('Invalid targetDeploymentId');
    const result = await this.deployments.rollback(projectId, targetDeploymentId);
    return { deploymentId: result.id, activePath: result.activePath };
  }
}
